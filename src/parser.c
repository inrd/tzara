...
int parseCreateNodeInstruction(void *tz, char **tokens, int numTokens,
  ...
  if (nodeType == INVALID_NODE_TYPE) {
    fprintf(stderr, "Could not create node : invalid node type...\n");
    return 1;
  }

  printf("Creating %s : %s\n", nodesDoc[nodeType].name, name);

  if (nodesDoc[nodeType].ctor != NULL) {
    created = nodesDoc[nodeType].ctor();
  } else {
    switch (nodeType) {
    case MODULE_NODE:
      created = parseAndCreateModule(tokens, numTokens, parentDir);
      break;
    case MATRIX_NODE:
      created = parseAndCreateMatrix(tokens, numTokens, parentDir);
      break;
    case MGET_NODE:
      created = parseAndCreateMget(tz, tokens, numTokens, isModule);
      break;
    case MSET_NODE:
      created = parseAndCreateMset(tz, tokens, numTokens, isModule);
      break;
    default:
      fprintf(stderr, "Could not create node : invalid node type...\n");
      return 1;
  ...
  err = addEngineNode(tz, created, name, isModule);
